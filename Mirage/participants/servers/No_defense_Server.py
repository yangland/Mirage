...
class No_defense_Server(BasicServer):
    ...
    def _warmup_bootstrap_triggers_for_all_regions(
        ...
        mali_test_loader,        # <<< add this
        logger
    ):
        for rid, real_id in canonical_client_for_region.items():
            if rid in malicious_client.trigger_set_by_region and rid in malicious_client.mask_set_by_region:
                continue
    ...
    def broadcast_upload(
        ...
        **kwargs
        ...
        rounds_since_start = iteration - self.params["start_iteration"]
        warmup_len = int(self.params.get("warmup_no_attack_iters", 0))
        if rounds_since_start < warmup_len:

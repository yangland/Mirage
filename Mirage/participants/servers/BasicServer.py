...
class \
    ...
    def select_clients(self, iteration, region_to_malicious_client):
        total_clients = self.params["no_of_total_participants"]
        total_adversaries = self.params["no_of_total_adversaries"]
        adversaries_per_iter = self.params["no_of_adversaries_per_iter"]
        clients_per_iter = self.params["no_of_participants_per_iteration"]
        strategy = self.params.get("clients_region_map", "by_order")

        adversary_id_pool = list(range(total_adversaries))
        benign_pool = list(range(total_adversaries, total_clients))
        ...
        if strategy == "pre_defined":
            ...
            if adversaries_per_iter % len(regions_to_attack) != 0:
                raise ValueError(
                    f"Cannot evenly assign {adversaries_per_iter} adversaries to {len(regions_to_attack)} regions."
                )

            num_attackers_per_region = adversaries_per_iter // len(regions_to_attack)

            for region in regions_to_attack:
                # Use the passed-in region-to-client map
                canonical_client = region_to_malicious_client[region]
                selected_malicious_clients.extend([canonical_client] * num_attackers_per_region)

        elif strategy == "random":
            region_ids = list(range(1, total_adversaries + 1))
            regions_to_attack = random.sample(region_ids, adversaries_per_iter)
            selected_malicious_clients = random.sample(adversary_id_pool, adversaries_per_iter)
        ...
        num_benign = clients_per_iter - len(selected_malicious_clients)
        selected_benign_clients = random.sample(benign_pool, num_benign)
